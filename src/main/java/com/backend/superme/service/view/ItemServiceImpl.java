...
public class ItemServiceImpl implements ItemService {
    ...
    // 정렬된 상품 목록을 반환하는 메서드
    @Override
    public List<ItemDto> sortItems(String sortBy) {
        List<Item> sortedItems;
        switch (sortBy) {
            case "nameAscending":
                sortedItems = itemRepository.findAllByOrderByNameAsc();
                break;
            case "nameDescending":
                sortedItems = itemRepository.findAllByOrderByNameDesc();
                break;
            case "priceAscending":
                sortedItems = itemRepository.findAllByOrderByPriceAsc();
                break;
            case "priceDescending":
                sortedItems = itemRepository.findAllByOrderByPriceDesc();
                break;
            case "registrationDateAscending":
                sortedItems = itemRepository.findAllByOrderByRegistrationDateAsc();
                break;
            case "registrationDateDescending":
                sortedItems = itemRepository.findAllByOrderByRegistrationDateDesc();
                break;
            default:
                sortedItems = itemRepository.findAllByOrderByNameAsc();
        }
